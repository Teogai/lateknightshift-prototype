...
- [2026-04-22] done: fix stun duration bug — removed `_decayStunStatuses()` from `startEnemyTurn()` (was causing double decay per turn cycle); stun now correctly lasts 2 turns on enemy pieces; added integration test `stun on enemy lasts exactly 2 enemy turns via turn cycle`; updated stun card description to 'Apply {stun} for 2 turns.' and keyword tooltip to 'Stun: cannot move for 2 turns.'; 348 tests green
...
- [2026-04-22] done: centralized pawn promotion system — js/engine2/promotion.js with getPawnsNeedingPromotion() and resolvePromotions(); teleport card now detects promotion; removed manual checks from playMoveCard/playKnightMoveCard/_playPatternMoveCard/playPawnBoostCard; enemy pawns auto-promote to queen on rank 1; updated docs/PROMOTION.md and ARCHITECTURE.md
- [2026-04-22] done: fix teleport promotion bug — centralized post-action promotion check via handlePostAction() in ui.js; all card handlers now use handlePostAction() instead of manual promotion checks; teleport no longer skips promotion; no phantom promotions on subsequent turns; 392 tests green
- [2026-04-22] done: fix reward placement square highlight — `.sq.selected` border was hidden by `.sq.summon-target` outline (same 3px space with box-sizing:border-box); added `.sq.summon-target.selected { outline-color: #f0e040; }` to css/room.css; added test verifying CSS rule exists; 393 tests green
- [2026-04-22] done: rename shop to piece_reward and replace all events with piece_reward — config/path.js ROOM_META/FIXED_PATH updated; js/ui.js removed event branch, changed shop→piece_reward; config/game.js roomWeights updated; tests updated (run.test.js, event_reward_ui.test.js); 395 tests green
- [2026-04-24] done: fix relic reward screen — select-then-confirm pattern, debug reroll button, .relic-grid/.relic-choice CSS styling; fix relic bar custom tooltip (not native title); add debug reroll to all 8 main choice screens (card/piece/relic/upgrade/transform/shop/charm/defeat); update callers in ui.js with reroll callbacks; 422 tests green (7 pre-existing run.test.js failures unrelated to changes)
